...
function buildEmailHtml(data: {
  ...
  const pathMatch = pagePath.match(/\/handyman\/([^/]+)/);
  ...
  const sourceLabel = isHomePage ? 'Homepage' : `${cityDisplay} — Handyman Services`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const html = buildEmailHtml({
    siteName: site_name || 'Handyman Services',
    fullName,
    email,
    phone: phone || '',
    comment: comment || '',
  ...
  try {
    await resend.emails.send({
      from: 'NKY Handyman Alerts <alerts@mursenmaintenance.com>',
